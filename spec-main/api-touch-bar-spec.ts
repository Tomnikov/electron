  ...
  it('throws an error if multiple OtherItemProxy items are added', () => {
    expect(() => {
      const touchBar = new TouchBar({
        items: [new TouchBarOtherItemsProxy(), new TouchBarOtherItemsProxy()],
      });
      touchBar.toString();
    ...
    it('can be added to and removed from a window', () => {
      ...
      const touchBar = new TouchBar({
        items: [
          ...
          new TouchBarGroup({
            items: new TouchBar({ items: [new TouchBarLabel({ label: 'hello' })] }),
          }),
          label,
          new TouchBarOtherItemsProxy(),
          new TouchBarPopover({
            items: new TouchBar({ items: [new TouchBarButton({ label: 'pop' })] }),
          }),
          new TouchBarSlider({
            label: 'slide',
            value: 5,
